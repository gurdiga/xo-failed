...
asyncTest('Încheiere de numire a datei nimicirii bunurilor', function() {
  ...
  var procedură = app.FormularProcedură.colectează(),
      bunuriNimiciteColectate = procedură['obiectul-urmăririi']['bunuri-nimicite'],
      bun;

  (function verificăColectarea() {
    equal(procedură['obiectul-urmăririi']['bunuri-nimicite'].length, numărDeBunuri, 'colectare: avem numărul corespunzător de bunuri');

    for (var i = 0; i < numărDeBunuri; i++) {
      bun = bunuriNimiciteColectate[i];
  ...
  $formular.one('salvat', function() {
    ...
    $formular.one('închis', function() {
      ...
      $formular.one('populat', function() {
        ...
        for (var i = 0; i < numărDeBunuri; i++) {
          equal($cîmpuriBunuriNimicite.eq(i).find('.etichetă').val(), bunuriNimiciteColectate[i].descrierea, 'populare: descrierea corespunde');
          equal($cîmpuriBunuriNimicite.eq(i).find('input').val(), bunuriNimiciteColectate[i].suma, 'populare: suma corespunde');
          equal($cîmpuriBunuriNimicite.eq(i).find('.valuta').val(), bunuriNimiciteColectate[i].valuta, 'populare: valuta corespunde');
        }
